...
Your craft orbits a star. All systems on board are, for all intents and purposes, electrical:
an ion drive (propulsion), mass driver (gun), and a reaction wheel (rotation).
Proximity to the star lets your solar panel harvest more energy.

The game uses real physics principles (e.g. gravity and radiative energy calculations)
where this is possible without compromising gameplay.

Inspired by KSpaceDuel[1] by Andreas Zehender (1998), this remake features:

 - Online multiplayer
 - Player settings are synchronised (based on who connects first), you do not have
   to join a specific server to play a certain configuration 
 - Good hitbox accuracy -- no more unexpected deaths 10km away from the star!
 - Health and energy levels visualized near the player rather than having to look over
   at a control panel
